...
impl NerRecognizer {
    ...
    pub fn from_config(config: NerConfig) -> Result<Self> {
        ...
        let session = if !config.model_path.is_empty() {
            ...
            if model_path.exists() {
                if Self::debug_enabled() {
                    eprintln!("[ner] building session for model: {}", config.model_path);
                }
                debug!("Loading ONNX model from: {}", config.model_path);
                let mut builder = Session::builder()?;
                if Self::debug_enabled() {
                    eprintln!("[ner] session builder created");
                }
                builder = builder
                    .with_execution_providers([ep::CPU::default().build().error_on_failure()])
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if Self::debug_enabled() {
                    eprintln!("[ner] CPU execution provider configured");
                }
                builder = builder
                    .with_optimization_level(GraphOptimizationLevel::Level1)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if Self::debug_enabled() {
                    eprintln!("[ner] optimization level configured");
                }
                builder = builder
                    .with_intra_threads(1)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if Self::debug_enabled() {
                    eprintln!("[ner] intra threads configured");
                    eprintln!("[ner] committing session from file");
                }
                match builder.commit_from_file(&config.model_path) {
                    Ok(s) => {
                        if Self::debug_enabled() {
                            eprintln!("[ner] session commit finished");
                        }
    ...
    fn infer(&self, input_ids: &[u32], attention_mask: &[u32]) -> Result<Vec<Vec<f32>>> {
        if Self::debug_enabled() {
            eprintln!("[ner] infer start: {} tokens", input_ids.len());
        }
        ...
        if Self::debug_enabled() {
            eprintln!("[ner] session.run start");
        }
        let outputs = session.run(inputs)?;
        if Self::debug_enabled() {
            eprintln!("[ner] session.run finished");
        }
