...
mod tests {
    ...
    #[test]
    fn run_redacts_pediatric_intake_questionnaire_fields_without_common_false_positives() {
        let temp = tempdir().unwrap();
        let input = temp.path().join("intake-questionnaire.md");
        fs::write(
            &input,
            "Client:\nJane Doe\nDOB:\n02/29/2016\nProvider:\nShina Halavi, PhD\nProvider License:\nPh. D. #12345\nName of person filling out this form: Tania Shah\nRelationship to patient: Mother\nPhone: 6143526898\nEmail: parent@example.com\nFull Name: Surina S. Shah\nSchool: Meadows Avenue Elementary\nRace/Ethnicity: Indian\nPrimary Language: English\nChild's place of birth:\nSanta Monica, CA\nYour child's primary physician/clinic:\nName: Jane Pediatrician\nAddress: Santa Monica CA\nPhone: 3105551212\nPlease list the persons who are currently in the home with the patient.\nMom - Tania 40, Dad - Neerav 42, sister - Alina 5\nLanguages\nFirst language: English\nPrimary language spoken in the home: English\nCaregiver #1\nName: Neerav Shah\nRelationship to patient: Father\nBirthplace: Los Angeles\nHighest degree/grade completed: MBA\nPlace of employment: Workday\nCurrent School/Teachers:\nSchool Name: Meadows Avenue Elementary\nPrimary teacher's name: Mrs. Gabriella\nTutor's name: Grace\nTeacher's email address: teacher@example.com\nTeacher's phone: 310-555-9999\nReferral from pediatrician office\nSurina is unable to do homework independently.\n",
        )
        .unwrap();
        ...
        assert!(!output.contains("02/29/2016"));
        assert!(!output.contains("Tania Shah"));
        assert!(!output.contains("6143526898"));
        assert!(!output.contains("Santa Monica, CA"));
        assert!(!output.contains("Neerav"));
        assert!(!output.contains("3105551212"));
        assert!(!output.contains("310-555-9999"));
        assert!(output.contains("[CLIENT]"));
        assert!(output.contains("[DATE_OF_BIRTH]"));
        assert!(output.contains("[PROVIDER]"));
        assert!(output.contains("[PHONE_NUMBER]"));
        assert!(output.contains("[EMAIL_ADDRESS]"));
        assert!(output.contains("[INSTITUTION]"));
        assert!(output.contains("[ADDRESS]"));
        assert!(output.contains("[FAMILY_MEMBER]"));
        assert!(output.contains("Race/Ethnicity: Indian"));
        assert!(output.contains("Primary Language: English"));
        assert!(output.contains("Highest degree/grade completed: MBA"));
        assert!(!output.contains("[ORGANIZATION]ina"));
    }
    ...
    #[test]
    fn run_redacts_change_of_name_form_entities_from_extracted_pdf_text() {
        let temp = tempdir().unwrap();
        let input = temp.path().join("name-change-form.txt");
        fs::write(
            &input,
            "Form Adopted for Mandatory Use\nORDER TO SHOW CAUSE—CHANGE OF NAME\nGenerated by Guide and File\nSelf-Represented\nEran Naor Boodnero\nEran Naor Boodnero\nEran Naor Boodnero\n24950 Via Florecer #7\neboodnero@gmail.com\n700 Civic Center Drive West\nSanta Ana, CA 92701\nCentral Justice Center\nOrange\nMission Viejo\n818-509-4253\nOrange County Register\nEran Naor Boodnero Eran Naor Botenero\nCA 92692\n",
        )
        .unwrap();
        ...
        let output = fs::read_to_string(summary.output_path.unwrap()).unwrap();
        assert!(!output.contains("Eran Naor Boodnero"));
        assert!(!output.contains("eboodnero@gmail.com"));
        assert!(!output.contains("818-509-4253"));
        assert!(!output.contains("700 Civic Center Drive West"));
        assert!(!output.contains("Santa Ana, CA 92701"));
        assert!(!output.contains("Mission Viejo"));
        assert!(!output.contains("Orange"));
        assert!(!output.contains("Orange County Register"));
        assert!(!output.contains("Central Justice Center"));
        assert!(!output.contains("Eran Naor Botenero"));
        assert!(output.contains("[CLIENT]"));
        assert!(output.contains("[EMAIL_ADDRESS]"));
        assert!(output.contains("[PHONE_NUMBER]"));
        assert!(output.contains("[ADDRESS]"));
    ...
    #[test]
    fn run_redacts_student_score_report_entities_from_extracted_pdf_text() {
        let temp = tempdir().unwrap();
        let input = temp.path().join("student-score-report.txt");
        fs::write(
            &input,
            "CDE Logo\nCalifornia Department of Education\nMEADOWS AVENUE ELEMENTARY\nMANHATTAN BEACH UNIFIED\nSSID: 8706108857\nSchool Year: 2024–25\nGrade: 3\nFOR THE FAMILY OF:\nSURINA S. SHAH\n1825 23RD ST\nMANHATTAN BEACH, CA 90266\nSurina S. Shah | 2024–25 Student Score Report\n",
        )
        .unwrap();
        ...
        let output = fs::read_to_string(summary.output_path.unwrap()).unwrap();
        assert!(!output.contains("SURINA S. SHAH"));
        assert!(!output.contains("1825 23RD ST"));
        assert!(!output.contains("MANHATTAN BEACH, CA 90266"));
        assert!(!output.contains("8706108857"));
        assert!(!output.contains("MEADOWS AVENUE ELEMENTARY"));
        assert!(!output.contains("MANHATTAN BEACH UNIFIED"));
        assert!(output.contains("[CLIENT]"));
        assert!(output.contains("[ADDRESS]"));
        assert!(output.contains("[STUDENT_ID]"));
        assert!(output.contains("[INSTITUTION]"));
    }
